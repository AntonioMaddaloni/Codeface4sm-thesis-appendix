...
load.mailinglist.graph <- function(conf, start.date, end.date) {
  ...
  ## --- CONTROLLO DI SICUREZZA: Se il grafo è vuoto, ritorna subito senza indicizzare ---
  if (vcount(mail.graph) == 0) {
    loginfo("Mailing list graph is empty for this range. Skipping indexing.")
    return(mail.graph)
  }
  ## -------------------------------------------------------------------------------------
  ...
  V(mail.graph)$name <- as.character(names)
  V(mail.graph)$id <- ids
  ## simplify the graph removing multiple edges and loops
...
sociotechnical.analysis <- function (sociotechdir, codedir, conf) {
  ...
  for (range in 1:length(ranges)) {
    ...
    # --- NUOVO CONTROLLO GRAFO VUOTO ---
    if (vcount(mail.graph) == 0) {
      loginfo("Skipping smell detection for empty mailing list graph.")
      
      mail.clusters <- list(membership=c(), names=c(), vcount=0)
      smell.org.silo <- list()
      smell.radiosilence <- c()
      smell.potential.black.cloud.orig <- list()
      smell.potential.black.cloud <- c()
      smell.black.cloud <- c()
      smell.primadonnas <- c()
      smell.missing.links <- list()
      
      all.smell.org.silo[range] <- 0
      all.smell.radiosilence[range] <- 0
      all.smell.blackcloud[range] <- 0
      all.smell.primadonnas[range] <- 0
      all.smell.missing[range] <- 0
      
      prev.range.potential.black.cloud <- c()
      
    } else {
      # Codice originale protetto dentro l'else
    ...
    # --- FINE CONTROLLO GRAFO VUOTO ---

        
    ## Compute socio-technical metrics
    if (vcount(mail.graph) == 0) {
      # Se il grafo mail è vuoto, forziamo le metriche mail a zero/NAs
      all.metric.congruence[range] <- 0
      all.metric.communicability[range] <- 0
      ...
      all.devs.mail.core[range] <- 0
      all.devs.global.core[range] <- length(devs.global.core)
      devs.ml.code.core <- c()
      all.devs.ml.code.core[range] <- 0
      all.devs.mail.only.core[range] <- 0
      all.devs.code.only.core[range] <- all.devs.code.core[range]
      
      all.metric.code.truck[range] <- ((length(V(code.graph)) - all.devs.code.core[range]) / length(V(code.graph)))
      all.metric.mail.truck[range] <- 0
      all.metric.global.truck[range] <- ((length(V(global.graph)) - all.devs.global.core[range]) / length(V(global.graph)))
      
    } else {
      # Codice originale se ci sono email
      ...
      ## truck number
      all.metric.code.truck[range] <- ((length(V(code.graph)) - all.devs.code.core[range]) / length(V(code.graph)))
      all.metric.mail.truck[range] <- ((length(V(mail.graph)) - all.devs.mail.core[range]) / length(V(mail.graph)))
      all.metric.global.truck[range] <- ((length(V(global.graph)) - all.devs.global.core[range]) / length(V(global.graph)))
    }
    ...
    ## (smelly developers)
    # --- SECONDO AGGIUSTAMENTO PER IL GRAFO MAIL VUOTO ---
    if (vcount(mail.graph) == 0) {
      all.metric.smelly.devs[range] <- 0
      prev.smelly.devs <- c()
    } else {
    ...
    # --- TERZO AGGIUSTAMENTO PER MODULARITÀ MAIL ---
    if (vcount(mail.graph) == 0) {
      all.metric.mail.modularity[range] <- 0
    } else {
    ...
    # --- GESTIONE GRAFO MAIL VUOTO PER I PLOT ---
    if (vcount(mail.graph) == 0) {
      
      # Pagina 1: Mail Graph vuoto
      plot(0:10, type="n", xaxt="n", yaxt="n", bty="n", xlab="", ylab="", main="Communication network from mailing lists")
      text(5, 5, "Mailing List Graph Empty for this range")
      
      # Pagina 2: Collaboration Graph (questo c'è sempre)
      plot.igraph(
      ...
      # Pagine dei Community Smells (stampiamo avvisi vuoti per quelli legati alle mail)
      plot(0:10, type="n", xaxt="n", yaxt="n", bty="n", xlab="", ylab="")
      text(5, 5, "Community smell: no organisational silo effect detected (No Mail)!")
      
      plot(0:10, type="n", xaxt="n", yaxt="n", bty="n", xlab="", ylab="")
      text(5, 5, "Community smell: no missing links detected (No Mail)!")
      
      plot(0:10, type="n", xaxt="n", yaxt="n", bty="n", xlab="", ylab="")
      text(5, 5, "Community smell: no prima-donnas effect detected (No Mail)!")
      
      plot(0:10, type="n", xaxt="n", yaxt="n", bty="n", xlab="", ylab="")
      text(5, 5, "Community smell: no black-cloud effect detected (No Mail)!")
      
      plot(0:10, type="n", xaxt="n", yaxt="n", bty="n", xlab="", ylab="")
      text(5, 5, "Community smell: no radio silence detected (No Mail)!")

    } else {
      # --- CODICE ORIGINALE SE IL GRAFO MAIL HA DEI NODI ---
      plot.igraph(
...
config.script.run({
  ...
  ## 1. Esegui l'analisi socio-tecnica principale (Genera il report.csv e i singoli grafi)
  sociotechnical.analysis(sociotechdir, codedir, conf)
  
  ## 2. Esegui il report statistico degli smells in modo protetto
  tryCatch({
    create.community.smells.report(sociotechdir, conf$project)
    cat("INFO: Report statistico degli smells generato con successo.\n")
  }, error = function(e) {
    cat("AVVISO: Salto il report statistico (varianza insufficiente nei range corti), ma l'analisi continua!\n")
  })
  
  ## 3. Esegui la generazione dei grafici globali in modo protetto (Risolve il problema di ylim)
  tryCatch({
    create.global.report.graphs(sociotechdir)
    cat("INFO: Grafici globali PDF generati con successo.\n")
  }, error = function(e) {
    cat("AVVISO: Impossibile tracciare alcune linee nei grafici globali per via dei valori NA nei range vuoti (errore ylim), ma lo script è terminato correttamente e il file report.csv è salvo!\n")
  })
  
  cat("\nANALYSER INFO: Processo terminato! Controlla la cartella di output.\n")
})